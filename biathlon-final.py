...
def hit_prob(prob):
'''funktionen hit probability - bestämmer träff/miss och ändrar träffsannolikheten (ts)'''
    tf = randint(0,100)
...
def shoot(n): #
'''funktionen låter spelaren bestämma target att skjuta på'''
    while True:
        target = input(f"\nShot nr {n} at target: ")
...
def main():
    prob = 55 # träffsannolikhet, probability
    n = 1 # skotträknare, counter
    while n < 6:
        sign()
        n_prob = hit_prob(prob) # (n_prob = new probability) kollar om det blir träff eller miss och tar emot nya sannolikheten
